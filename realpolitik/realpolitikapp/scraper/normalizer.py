...
# TKTK better error handling for skip
# like a checksum
...
def venue_join(event, venue):
  event_venue_name = event['venue-name']
  venue_venue_name = venue['venue-name']
  ## TKTK use tokenizer instead of substring compare
  ((event_venue_name in venue_venue_name) |
   (venue_venue_name in event_venue_name)) &
  event['city'] is venue['city']
# TK add duration
